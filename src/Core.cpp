...
#include "marioKart/marioKart.hpp"
...
Arcade::Core::Core()
{
    ...
    _deltaTime = 0;
    _hasAGame = true; //from false
    _gameModule = std::make_unique<MarioKart>();
    _actualGame = "Menu";
    _graphicalModule = std::make_unique<SFML>();
    for (int i = 0; i < 3; i += 1) {
        _username[i] = 'A';
        _validChar[i] = 0;
    }
...
{
    return {}; //removed from the original code
}
